...
(ported from Google gdata library by Robin Gottfried <google@kebet.cz>)
"""

# TODO: Use relative import for http_core
...
def _to_uri(str_or_uri):
  if isinstance(str_or_uri, (str, unicode)):
    return http_core.Uri.parse_uri(str_or_uri)
  return str_or_uri


def generate_auth_sub_url(next, scopes, secure=False, session=True,
    request_url=http_core.parse_uri(
  ...
  Args:
    next: http_core.Uri or string The URL user will be sent to after
          authorizing this web application to access their data.
    scopes: list containint strings or http_core.Uri objects. The URLs
            of the services to be accessed. Could also be a single string
            or single http_core.Uri for requesting just one scope.
    secure: boolean (optional) Determines whether or not the issued token
            is a secure token.
    session: boolean (optional) Determines whether or not the issued token
             can be upgraded to a session token.
    request_url: http_core.Uri or str The beginning of the request URL.
                 This is normally
  ...
  if isinstance(request_url, (str, unicode)):
    request_url = http_core.Uri.parse_uri(request_url)
...
def auth_sub_string_from_url(url, scopes_param_prefix='auth_sub_scopes'):
  ...
  Returns:
    ...
    (as http_core.Uri objects) which are URL prefixes under which this
  ...
  if isinstance(url, (str, unicode)):
    url = http_core.Uri.parse_uri(url)
...
class AuthSubToken(object):
  ...
  def from_url(str_or_uri):
    ...
    Args:
      str_or_uri: The current page's URL (as a str or http_core.Uri)
...
class SecureAuthSubToken(AuthSubToken):
  ...
  def modify_request(self, http_request):
    ...
    Args:
      http_request: The http_core.HttpRequest which contains all of the
...
def generate_request_for_request_token(
  ...
  Returns:
    An http_core.HttpRequest object with the URL, Authorization header
    and body filled in.
  """
  request = http_core.HttpRequest(auth_server_url, 'POST')
...
def generate_request_for_access_token(
  ...
  """
  http_request = http_core.HttpRequest(auth_server_url, 'POST')
...
def generate_oauth_authorization_url(
  ...
  Returns:
    An http_core.Uri pointing to the token authorization page where the
    user may allow or deny this app to access their Google data.
  """
  uri = http_core.Uri.parse_uri(auth_server)
...
class OAuth2Token(object):
  ...
  def _refresh(self, request):
    ...
    Args:
      request: The http_core.HttpRequest which contains all of the
  ...
  def get_access_token(self, code):
    ...
    http_client = http_core.HttpClient()
    http_request = http_core.HttpRequest(uri=self.token_uri, method='POST',
                                              headers=headers)
  ...
  def revoke(self, revoke_uri=None, refresh_token=None):
    ...
    revoke_uri = http_core.ParseUri(base_revoke_uri)
    revoke_uri.query['token'] = refresh_token
    http_client = http_core.HttpClient()
    http_request = http_core.HttpRequest(uri=revoke_uri, method='GET')
    response = http_client.request(http_request)
    if response.status != 200:
...
class OAuth2TokenFromCredentials(OAuth2Token):
  ...
  def _refresh(self, unused_request):
    ...
    Args:
      unused_request: The http_core.HttpRequest which contains all of the
